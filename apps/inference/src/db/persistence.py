...
def store_inference_partial_artifacts(
    report_id: int,
    *,
    detections: Any,
    landmarks: Any,
...
def list_pdf_object_keys_for_report(report_id: int) -> list[str]:
    with get_conn() as conn:
        cur = conn.cursor()
        cur.execute(
            "SELECT pdf_object_key FROM pdf_reports WHERE report_id = %s",
            (report_id,),
        )
        rows = cur.fetchall() or []
    out: list[str] = []
    for r in rows:
        k = r.get("pdf_object_key") if isinstance(r, dict) else r[0]
        if k:
            s = str(k).strip()
            if s:
                out.append(s)
