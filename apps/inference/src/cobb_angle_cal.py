...
def _create_angles_dict(pt, mt, tl):
  """Each of pt, mt, tl: (angle, [top_idx, bottom_idx])."""
  return {
    "pt": {
        "angle": pt[0],
        "idxs": [pt[1][0], pt[1][1]],
...
def cobb_angle_cal(landmark_xy, image_shape):
  """Cobb from interlaced x/y keypoints; `image_shape[0]` is height. Returns cobb_angles_list, angles_with_pos, curve_type, midpoint_lines."""
  landmark_xy = list(landmark_xy)
  ap_num = int(len(landmark_xy)/2)
  vnum = int(ap_num / 4)
...
def keypoints_to_landmark_xy(keypoints):
    """Per-vertebra `[[x,y],...]` lists to `[x1..xn, y1..yn]` for Cobb input."""
    x_points = []
