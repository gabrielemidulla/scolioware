...
from src.ingest.dicom_ingest import decode_upload_to_jpeg
from src.queue.rq_tasks import enqueue_process_report
...
def _attach_report_artifacts(out: dict[str, Any], row: dict[str, Any], *, omit_null_extras: bool) -> None:
    ...
    for k in _REPORT_EXTRA_FIELDS:
        if k not in row:
            continue
        val = row[k]
        if omit_null_extras and val is None:
            continue
        out[k] = _json_scalar(val)
...
async def enqueue_scan(
    ...
    if not dashboard_storage.storage_proxy_configured():
        raise HTTPException(
            status_code=503,
            detail="Dashboard storage proxy is not configured (set DASHBOARD_STORAGE_URL on inference).",
        )
    ...
    key = dashboard_storage.object_key_original(patient_id, report_id)
    ...
    try:
        dashboard_storage.put_bytes(key, jpeg, content_type="image/jpeg")
...
async def get_report(report_id: int):
    ...
    st = row["status"]
    if st == "completed":
        _attach_report_artifacts(out, row, omit_null_extras=False)
    elif st == "failed":
        _attach_report_artifacts(out, row, omit_null_extras=True)
    return out
